...
function initializeDataTable() {
  ...
  // Style the search input using Bootstrap classes
  ...
  if (searchInput) {
    ...
    if(searchLabel) {
        // Wrap input in an input-group for better styling
        const inputGroup = document.createElement('div');
        inputGroup.classList.add('input-group', 'input-group-sm', 'mb-2');
        searchLabel.parentNode.insertBefore(inputGroup, searchLabel);
        inputGroup.appendChild(searchLabel);
        inputGroup.appendChild(searchInput);
        searchLabel.classList.add('input-group-text');
        searchLabel.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
